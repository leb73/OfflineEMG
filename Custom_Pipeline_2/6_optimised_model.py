...
print("=" * 60)
print("  6_optimised_model.py - CNN-LSTM Hyperparameter Grid Search")
print("=" * 60)
...
class EMDEarlyStopping(callbacks.Callback):
    def __init__(self, X_val, Y_val_enc, times_val, patience=10):
        super().__init__()
        self.X_val = X_val
        self.Y_val_enc = Y_val_enc
        self.times_val = times_val
        self.patience = patience
        self.emd_mask = (self.times_val > EMD_START) & (self.times_val <= EMD_END)
        self.best_emd_acc = -1.0
        self.wait = 0
        self.best_weights = None
        self.best_epoch = 0
        
    def on_epoch_end(self, epoch, logs=None):
        if not np.any(self.emd_mask):
            return
            
        preds = self.model.predict(self.X_val, verbose=0)
        pred_classes = np.argmax(preds[:, -1, :], axis=-1)
        
        y_true_emd = self.Y_val_enc[self.emd_mask]
        y_pred_emd = pred_classes[self.emd_mask]
        
        emd_acc = accuracy_score(y_true_emd, y_pred_emd)
        if logs is not None:
            logs['emd_acc'] = emd_acc
            
        if emd_acc > self.best_emd_acc:
            self.best_emd_acc = emd_acc
            self.wait = 0
...
# ============================================================================
def main():
    print("Starting Model Optimisation...")
    script_dir = os.path.dirname(os.path.abspath(__file__))
    base_dir = os.path.join(os.path.dirname(script_dir), "extracted_trials_shifted")
    
    timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    out_dir = os.path.join(script_dir, "Offline_Training_Results", timestamp + "_Model_Optimisation")
    ...
    # Split into 85% trainval and 15% test (trial-level split)
    trainval_ds, test_ds = collections.defaultdict(list), collections.defaultdict(list)
    ...
    for run_mode in ["All_Features"]:
        ...
        feat_idx = 0 if run_mode == "All_Features" else 1
        
        test_raw_dict = {cls_name: [(item[feat_idx], item[3], item[4]) for item in sublist] for cls_name, sublist in test_ds.items()}
        
        # Prepare trainval trials per class for k-fold splitting
        trainval_trials_by_class = {}
        for cls_name, items in trainval_ds.items():
            trainval_trials_by_class[cls_name] = [(item[feat_idx], item[3], item[4]) for item in items]
        ...
        grid_max_epochs = [50, 100]
        
        keys = list(grid.keys())
        combinations = list(itertools.product(*grid.values()))
        total_configs = len(combinations) * len(grid_max_epochs)
        
        print(f"\nStarting {N_FOLDS}-Fold CV Grid Search. Total configurations: {total_configs}")
        
        best_mean_emd_acc = -1.0
        best_params = {}
        best_max_epochs = 100
        best_median_epoch = 20
        
        results = []
        config_idx = 0
        
        for max_ep in grid_max_epochs:
            for values in combinations:
                params = dict(zip(keys, values))
                config_idx += 1
                print(f"\n--- Config {config_idx}/{total_configs} | max_epochs={max_ep} ---")
                ...
                for fold_i in range(N_FOLDS):
                    # Create fold-specific train/val split from trainval trials
                    fold_train_raw = []
                    fold_val_raw = []
                    
                    for cls_name, trials in trainval_trials_by_class.items():
                        n_cls = len(trials)
                        fold_size = max(1, n_cls // N_FOLDS)
                        val_start = fold_i * fold_size
                        val_end = min(val_start + fold_size, n_cls)
                        
                        for t_i, trial in enumerate(trials):
                            if val_start <= t_i < val_end:
                                fold_val_raw.append(trial)
                            else:
                                fold_train_raw.append(trial)
                    
                    # Fit scaler on this fold's training data
                    fold_scaler = StandardScaler()
                    fold_train_feats = np.vstack([t[0] for t in fold_train_raw])
                    fold_scaler.fit(fold_train_feats)
                    
                    X_tr, Y_tr_raw, _ = build_sequences(fold_train_raw, fold_scaler, restrict_to_emd=False)
                    X_vl, Y_vl_raw, times_vl = build_sequences(fold_val_raw, fold_scaler, restrict_to_emd=False)
                    
                    Y_tr_enc = le.transform(Y_tr_raw)
                    Y_vl_enc = le.transform(Y_vl_raw)
                    
                    Y_tr_seq = np.repeat(Y_tr_enc[:, np.newaxis], SEQUENCE_LENGTH, axis=1)
                    Y_vl_seq = np.repeat(Y_vl_enc[:, np.newaxis], SEQUENCE_LENGTH, axis=1)
                    
                    input_shape = (X_tr.shape[1], X_tr.shape[2])
                    num_classes = len(all_classes)
                    
                    tf.keras.backend.clear_session()
                    model = create_cnn_lstm(input_shape, num_classes, **params)
                    
                    early_stop = EMDEarlyStopping(X_vl, Y_vl_enc, times_vl, patience=10)
                    
                    model.fit(
                        X_tr, Y_tr_seq,
                        validation_data=(X_vl, Y_vl_seq),
                        epochs=max_ep,
                        batch_size=8,
                        verbose=0,
                    ...
                    val_loss, val_acc = model.evaluate(X_vl, Y_vl_seq, verbose=0)
                    fold_emd_accs.append(early_stop.best_emd_acc)
                    fold_best_epochs.append(early_stop.best_epoch + 1)
                    fold_val_losses.append(val_loss)
                    fold_val_accs.append(val_acc)
                
                mean_emd = np.mean(fold_emd_accs)
                std_emd = np.std(fold_emd_accs)
                median_epoch = int(np.median(fold_best_epochs))
                print(f"  Mean EMD Acc: {mean_emd*100:.2f}% (+/- {std_emd*100:.2f}%), Median Best Epoch: {median_epoch}")
                
                res_dict = params.copy()
                res_dict['max_epochs'] = max_ep
                res_dict['mean_val_loss'] = np.mean(fold_val_losses)
                res_dict['mean_val_acc'] = np.mean(fold_val_accs)
                res_dict['mean_emd_acc'] = mean_emd
                res_dict['std_emd_acc'] = std_emd
                res_dict['median_best_epoch'] = median_epoch
                res_dict['fold_emd_accs'] = str(fold_emd_accs)
                results.append(res_dict)
                
                if mean_emd > best_mean_emd_acc:
                    best_mean_emd_acc = mean_emd
                    best_params = params
                    best_max_epochs = max_ep
                    best_median_epoch = median_epoch
                    print(">>> New Best Config! <<<")
        
        print(f"\nGrid Search Complete!")
        print(f"Best Params: {best_params}, max_epochs={best_max_epochs}")
        print(f"Mean EMD Acc: {best_mean_emd_acc*100:.2f}%, Median Best Epoch: {best_median_epoch}")
        
        # Save grid search results
        df_results = pd.DataFrame(results)
        df_results.to_csv(os.path.join(current_out_dir, "grid_search_results.csv"), index=False)
        
        # ====================================================================
        # FINAL RETRAIN WITH EMD EARLY STOPPING
        # ====================================================================
        # Use all trainval data, with a 90/10 internal split for early stopping
        all_trainval_raw = []
        for cls_name, trials in trainval_trials_by_class.items():
            all_trainval_raw.extend(trials)
        
        # Fit scaler on ALL trainval data
        scaler = StandardScaler()
        all_trainval_feats = np.vstack([t[0] for t in all_trainval_raw])
        scaler.fit(all_trainval_feats)
        
        # Split trainval into 90% retrain / 10% holdout for early stopping
        retrain_raw = []
        holdout_raw = []
        for cls_name, trials in trainval_trials_by_class.items():
            n_cls = len(trials)
            n_holdout = max(1, int(0.10 * n_cls))
            holdout_raw.extend(trials[-n_holdout:])
            retrain_raw.extend(trials[:-n_holdout])
        
        X_retrain, Y_retrain_raw, _ = build_sequences(retrain_raw, scaler, restrict_to_emd=False)
        X_holdout, Y_holdout_raw, times_holdout = build_sequences(holdout_raw, scaler, restrict_to_emd=False)
        
        Y_retrain_enc = le.transform(Y_retrain_raw)
        Y_holdout_enc = le.transform(Y_holdout_raw)
        
        Y_retrain_seq = np.repeat(Y_retrain_enc[:, np.newaxis], SEQUENCE_LENGTH, axis=1)
        Y_holdout_seq = np.repeat(Y_holdout_enc[:, np.newaxis], SEQUENCE_LENGTH, axis=1)
        
        input_shape = (X_retrain.shape[1], X_retrain.shape[2])
        num_classes = len(all_classes)
        
        print(f"\nRetraining Best Model with EMD Early Stopping...")
        print(f"  Retrain: {X_retrain.shape[0]} samples, Holdout: {X_holdout.shape[0]} samples")
        print(f"  Best params: {best_params}")
        
        tf.keras.backend.clear_session()
        final_best_model = create_cnn_lstm(input_shape, num_classes, **best_params)
        
        retrain_early_stop = EMDEarlyStopping(X_holdout, Y_holdout_enc, times_holdout, patience=15)
        
        final_best_model.fit(
            X_retrain, Y_retrain_seq,
            validation_data=(X_holdout, Y_holdout_seq),
            epochs=150,
            batch_size=8,
            verbose=1,
        ...
        final_retrain_epoch = retrain_early_stop.best_epoch + 1
        final_retrain_emd_acc = retrain_early_stop.best_emd_acc
        print(f"  Final retrain stopped at epoch {final_retrain_epoch} with holdout EMD Acc: {final_retrain_emd_acc*100:.2f}%")
        
        trained_models = {"Best_CNN-LSTM": final_best_model}
        ...
        summary_text = (
            ...
            f"Run Mode: {run_mode}\n"
            f"Grid Search: {N_FOLDS}-Fold CV, {total_configs} configs\n"
            f"Best Params: {best_params}\n"
            f"CV Mean EMD Acc: {best_mean_emd_acc*100:.2f}%\n"
            f"Final Retrain: stopped at epoch {final_retrain_epoch}, holdout EMD Acc: {final_retrain_emd_acc*100:.2f}%\n"
            f"{'='*60}\n"
        )
