...
# ============================================================================
def main():
    print("Starting MFCC Comparison...")
    script_dir = os.path.dirname(os.path.abspath(__file__))
    base_dir = os.path.join(os.path.dirname(script_dir), "extracted_trials_shifted")
    
    timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    out_dir = os.path.join(script_dir, "Offline_Training_Results", timestamp + "_MFCC_Comparison")
    ...
    for run_mode in ["All_Features", "No_MFCC"]:
        print(f"\n" + "="*50)
        ...
        # We must create this to consume the exact same sequence of random numbers as the original script!
        _ = create_cnn_lstm_attn(input_shape, num_classes)
        ...
        for m_name, model in models_to_test.items():
            ...
            model.fit(X_full, Y_full_seq, epochs=20, batch_size=8, verbose=1)
