...
def plot_continuous_spectrograms(mean_specs, unique_times_by_class, out_dir):
    ...
    for class_idx, class_name in enumerate(classes):
        ...
        # Build continuous spectrogram array for each sensor:
        # Stack the mean spectrogram at each time step side by side.
        # Each window's STFT is (128 freq bins, 9 time frames).
        # We take the centre time frame (index 4) to get one column per window,
        # giving a (128, n_windows) image spanning the full time range.
        ...
        for w_idx, t_val in enumerate(unique_times):
            ...
            # Use the mean across the 9 STFT time frames for a cleaner image
        ...
        for sensor_idx in range(N_SENSORS):
            sensor_spec = continuous[:, :, sensor_idx]  # (128, n_windows)

            fig, ax = plt.subplots(figsize=(14, 4))
            im = ax.imshow(sensor_spec, aspect='auto', origin='lower',
                           cmap='magma', interpolation='bilinear',
                           extent=[time_array[0], time_array[-1], 0, f_max])
            ax.set_ylim(0, 128.0)
        ...
        for sensor_idx in range(N_SENSORS):
            ...
            vmin, vmax = sensor_spec.min(), sensor_spec.max()
            im = ax.imshow(sensor_spec, aspect='auto', origin='lower',
                           cmap='magma', interpolation='bilinear',
                           extent=[time_array[0], time_array[-1], 0, f_max],
                           vmin=vmin, vmax=vmax)
