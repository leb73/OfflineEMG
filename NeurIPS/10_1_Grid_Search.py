...
BASE_RESULTS_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "Offline_Training_Results", timestamp)
...
def plot_average_spectrograms(raw_stfts_by_class, raw_times_by_class, out_dir, nperseg, max_freq):
    ...
    for class_idx, class_name in enumerate(classes):
        ...
        for t_val in unique_times:
            ...
            fig.suptitle(f"{label_short} — Avg Spectrogram t = {t_val:+.2f}s (n={np.sum(mask)})", fontsize=11, fontweight='bold')
            spec_mod = np.mean(mean_spec[:, :, emg_ch_idx], axis=-1)
            spec_mod_sliced = spec_mod[:limit_idx, :]
            
            im = ax.imshow(spec_mod_sliced, aspect='auto', origin='lower', cmap='magma', interpolation='nearest', extent=[0, spec_mod_sliced.shape[1], 0, max_freq])
            ax.set_title("EMG", fontsize=11)
            ax.set_xlabel("Time frame", fontsize=9)
            ax.set_ylabel("Frequency (Hz)", fontsize=9)
            plt.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
            plt.tight_layout()
            plt.savefig(os.path.join(class_out_dir, f"avg_spec_{t_val:+.2f}s.png"), dpi=100)
            plt.close(fig)

# ========================================================================
# MODEL ARCHITECTURE (EMGNet)
# ========================================================================
class EMGNetClassifier(nn.Module):
    def __init__(self, num_channels=8, num_frames=9, num_frequencies=128, num_classes=7):
        ...
        self.encoder = nn.Sequential(
            nn.Conv2d(num_channels, 32, kernel_size=(3, 3), stride=1, padding='same', bias=False),
            nn.BatchNorm2d(32),
        ...
        self.decoder = nn.Sequential(
            nn.Conv2d(256, 256, kernel_size=(3, 3), stride=1, padding='same', bias=False),
            nn.BatchNorm2d(256),
            nn.ReLU(inplace=False),
            nn.Upsample(size=(max(1, num_frames // 4), 1), mode='bilinear', align_corners=False),
            
            nn.Conv2d(256, 128, kernel_size=(3, 3), stride=1, padding='same', bias=False),
            nn.BatchNorm2d(128),
            nn.ReLU(inplace=False),
            nn.Upsample(size=(max(1, num_frames // 2), 1), mode='bilinear', align_corners=False),
            
            nn.Conv2d(128, 32, kernel_size=(3, 3), stride=1, padding='same', bias=False),
            nn.BatchNorm2d(32),
            nn.ReLU(inplace=False),
...
def run_experiment(nperseg, noverlap, max_freq, results_dir):
    print(f"\n{'='*60}\nStarting Experiment: nperseg={nperseg}, noverlap={noverlap}, max_freq={max_freq}\n{'='*60}")
    ...
    plot_average_spectrograms(raw_stfts_by_class, raw_times_by_class, avg_spec_dir, nperseg, max_freq)
    ...
    im = ax.imshow(spec_sliced, aspect='auto', origin='lower', cmap='magma', extent=[0, spec_sliced.shape[1], 0, max_freq])
    ...
    print(f"Done evaluating config nperseg={nperseg}, freq={max_freq}.")


if __name__ == "__main__":
    stft_params = [
        {"nperseg": 128, "noverlap": 112},  # 16 step
        {"nperseg": 256, "noverlap": 240},  # 16 step (baseline)
        {"nperseg": 512, "noverlap": 496},  # 16 step
    ]
    freq_params = [128.0, 256.0, 512.0, 962.0]
    
    grid_configs = []
    for sp, fp in itertools.product(stft_params, freq_params):
        grid_configs.append({
            "nperseg": sp["nperseg"],
    ...
    print(f"Starting Grid Search with {len(grid_configs)} configurations.")
    for config in grid_configs:
        config_dir = os.path.join(BASE_RESULTS_DIR, f"Config_nperseg{config['nperseg']}_freq{config['max_freq']}")
        run_experiment(config["nperseg"], config["noverlap"], config["max_freq"], config_dir)
        
    print(f"\nAll configurations completed. Results saved to: {BASE_RESULTS_DIR}")
